...
![這相當容易。 你給我們源表，把一切都留給我們。 我們為你們提供美麗的適應性形式。 你總是可以對輸出進行小修小補，讓你滿意。 ](assets/pdf-to-adaptive-form-gitx50.gif)
...
該服務還支援啟用Adobe Sign的PDF forms。 如果源 PDF 表單有 Adobe Sign 文字標記，則此服務會保留轉換期間所有 Adobe Sign 的相關資訊，並將源 PDF 中顯示的簽署者資訊與對應的最適化表單欄位建立關聯。 此功能僅適用於 AcroForms。

該服務可將英語、法語、德語、西班牙語、義大利語和葡萄牙語格式轉換為適應形式。 您還可以使用 [AEM翻譯工作流](https://helpx.adobe.com/experience-manager/6-5/forms/using/using-aem-translation-workflow-to-localize-adaptive-forms.html)。
...
* 不要以英語、法語、德語、西班牙語、義大利語和葡萄牙語以外的任何語言上傳掃描、著色、填充的表單和表單。 此服務不支援這些表單。
...
現實世界表單在欄位佈局、命名或隱含建議方面可能具有複雜的資料捕獲要求，而基於AI/ML的檢測邏輯可能無法準確捕獲這些要求。 自動轉換完成後，您可以使用[檢閱和修正編輯器](review-correct-ui-edited.md)來檢查轉換後的表格並進行必要的更新，使成果更佳並更能提供符合期望的體驗。 完成必要的修改後，再一次轉換表單。

自動轉換所花費的時間取決於各種因素，如輸入表單的大小、表單的複雜性、對服務處理隊列的借出等。 使用者可透過資料夾 / 檔案上的狀態顯示器時時瞭解進度。 轉換完成後，使用者設定的電子郵件地址也會收到電子郵件通知。
