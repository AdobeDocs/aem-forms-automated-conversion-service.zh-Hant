...
1. [下載並安裝AEM6.4或AEM6.5](#aemquickstart)
...
### 下載並AEM安裝6.4AEM或6.5 {#aemquickstart}


automated forms conversion服務在作者實AEM例上執行。 您需AEM要6.4或AEM6.5才能設定作AEM者例項。 如果您尚未啟AEM動並執行，請從下列位置下載：
...
下載後AEM，如需設定作者例項的AEM指示，請參閱[部署和維護](https://helpx.adobe.com/experience-manager/6-5/sites/deploying/using/deploy.html#defaultlocalinstall)。
...
### 下載並安裝AEM Forms附加軟體包{#downloadaemformsaddon}
...
該連接器軟體包提供對[自動檢測邏輯部分](convert-existing-forms-to-adaptive-forms.md#run-the-conversion)功能和在AFC-2020.03.1版中提供的改進的早期訪問。如果您不需要AFC-2020.03.1中提供的功能和改進，請勿安裝軟體包。 您可以從Package Share](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq650/featurepack/AFCS-Connector-2020.03.1)下載連AEM接器套件。[


### 建立自訂主題和範本{#referencepackage}

如果您在AEM[生產模式](https://helpx.adobe.com/experience-manager/6-5/sites/administering/using/production-ready.html)(nosamplecontent runmode)中啟動，則不會安裝參考軟體包。 參考套件包含範例主題和範本。 automated forms conversion服務至少需要一個主題和一個模板，才能將PDF forms轉換為自適應表單。 建立您自己的自訂主題和範本，並指向[service configuration](#configure-the-cloud-service)使用服務前，使用自訂範本和主題。

## 設定服務 {#configure-the-service}

在您繼續設定服務並連線您的本機執行個體與Adobe雲上執行的服務之前，請先瞭解連線至服務所需的角色和權限。 本服務使用兩種不同的角色類型：管理員和開發人員：

* **管理員**:管理員負責管理組織的Adobe軟體和服務。管理員授與其組織中開發人員的存取權，以連線至在Adobe雲端執行的Automated forms conversion服務。 為組織布建管理員時，管理員會收到標題為&#x200B;**[!UICONTROL 'You now have administrator rights to manage Adobe software and services for your organization']**&#x200B;的電子郵件。 如果您是管理員，請檢查郵箱中是否有前述標題的電子郵件，然後繼續[授予您組織的開發人員的存取權。](#adduseranddevs)

![管理員存取權授予電子郵件](assets/admin-console-adobe-io-access-grantedx75.png)

* **開發人員**:開發人員會將本機AEM Forms作者例項連結至在Adobe雲端執行的Automated forms conversion服務。當管理員授與開發人員連線至Automated forms conversion服務的權限時，您現在可以存取「您」標題的電子郵件會傳送給開發人員，以管理您組織的AdobeAPI整合。 如果您是開發人員，請檢查郵箱中是否有前述標題的電子郵件，然後繼續[將您的本AEM地實例連接到Adobe雲上的Automated forms conversion服務。](#connectafcadobeio)
...
1. 點選&#x200B;**[!UICONTROL Download Public Key]**&#x200B;並將&#x200B;*AEM-Adobe-IMS.crt*&#x200B;憑證檔案儲存在您的電腦上。 證書檔案用於[在AdobeDevloper Console](#createintegration)上配置服務API。 點選&#x200B;**[!UICONTROL Next]**。
...
#### 在Adobe設備控制台{#createintegration}上配置服務API
...
建立雲端服務設定，將您的例項連AEM接至轉換服務。 它也可讓您指定轉換的範本、主題和表單片段。 您可以針對每組表單分別建立多個雲端服務組態。 例如，您可以為銷售部門表單設定個別設定，並為客戶支援表單設定個別設定。 執行下列步驟以建立雲端服務設定：
...
1. 點選&#x200B;**[!UICONTROL Global]**&#x200B;資料夾並點選&#x200B;**[!UICONTROL Create]**。 將顯示建立Automated forms conversion配置的頁。 配置在「全局」資料夾中建立。 您也可以在已存在的不同資料夾中建立配置，或為配置建立新資料夾。
...
1. 在&#x200B;**[!UICONTROL Create Automated Forms Conversion Configuration]**&#x200B;頁面的&#x200B;**[!UICONTROL Advanced]**&#x200B;標籤中，指定下列欄位的值：
   ...
   <tr>
   <td>啟動 Analytics</td>
   <td>選取選項，在所有轉換的表格上啟用Adobe Analytics。 在使用選項之前，請確定已為您的AEM Forms實例啟用Adobe Analytics。</td>
   </tr>
   </tbody>
   </table>
