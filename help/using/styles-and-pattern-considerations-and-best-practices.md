---
title: 最佳實務和考量事項
description: 自動錶單轉換服務(AFCS)的最佳實務和考量事項
...
本檔案提供表單管理員、作者和開發人員在使用[!DNL Automated Forms Conversion service] (AFCS)時可受益的准則和建議。 此影片探討最佳實務，從準備來源表單，到修正需要加倍努力進行自動化轉換的複雜模式。 這些最佳實務共同貢獻[!DNL Automated Forms Conversion service] (AFCS)的整體效能和輸出。

## 最佳做法

轉換服務會將您AEM [!DNL Forms]執行個體上可用的PDF forms轉換為最適化表單。 下列最佳實務可協助您改善轉換速度和準確性。 此外，這些最佳實務可協助您節省在轉換活動後所花費的時間。
...
* 請勿上傳[PDF產品組合](https://helpx.adobe.com/tw/acrobat/using/overview-pdf-portfolios.html)。 此服務無法將PDF Portfolio轉換為最適化表單。
...
* **AEM Forms as a Cloud Service：**&#x200B;預設範本和主題可供使用；您可以使用它們或建立自訂範本和主題。
* **AEM 6.5和AEM 6.5 LTS：**&#x200B;建立最適化表單範本和主題（或安裝參考資產，如[設定服務](configure-service.md#referencepackage)中所述）。 若要使用以核心元件為基礎的範本和主題，您必須啟用[最適化表單核心元件](https://experienceleague.adobe.com/docs/experience-manager-65/forms/adaptive-forms-core-components/enable-adaptive-forms-core-components.html?lang=zh-Hant)。
...
AEM [!DNL Forms Automated Conversion service]使用人工智慧和機器學習演演算法來瞭解來源表單的版面配置和欄位。 每個機器學習服務都會持續學習來源資料，並在每次流失時產生改良的輸出。 這些服務就像人類一樣，從經驗中學習。

[!DNL Automated Forms Conversion service]已接受大量表單的訓練。 它可輕鬆識別來源表單中的欄位並產生調適型表單。 不過，PDF forms中有些欄位和樣式肉眼很容易看見，但服務上卻難以理解。 此服務可將與適用欄位型別或面板不同的欄位或樣式指派給某些欄位或樣式。 以下列出所有此類欄位和樣式模式。
...
| 模式 | 解決方法 |
|--- |--- |
| **模式** <br>包含表單欄位的清單已合併或未轉換為對應的調適型表單元件&#x200B;<br><br>**解決方法** <br>使用具有明確邊界的表單欄位，或使用檢閱和修正編輯器來修正此類問題。 | 包含選擇群組的![清單](assets/best-practice-lists-containing-form-fields.png) |
| **模式** <br>服務可能會留下一些未識別的巢狀清單&#x200B;<br><br>**解決方法** <br>使用檢閱和修正編輯器來修正此類問題。 | 包含選擇群組的![清單](assets/best-practice-nested-lists.png) |
| **模式** <br>服務會合併一些包含彼此選擇群組的清單&#x200B;<br><br>**解決方法** <br>使用檢閱和修正編輯器來修正此類問題。 | 包含選擇群組的![清單](assets/best-practice-check-box-in-table-cells.png) |
