...
1. 導覽至http://[伺服器]：[連線埠]/system/console/bundles ，然後按一下 **[!UICONTROL Install/Update]**.
...
1. 建立 **中繼資料** 資料庫綱要中的表格，使用下列SQL敘述句：
...
1. 移至AEM Web Console設定頁面，位於 *http://[主機]：[連線埠]/system/console/configMgr*.
...
1. 前往位於AEM封裝管理員 *http://[主機]：[連線埠]/crx/packmgr/*.
...
1. 在編輯模式中開啟最適化表單。
1. 點選表單容器並選取設定 ![設定自適應表單](assets/configure-adaptive-form.png).
...
1. 插入 **[!UICONTROL Search & Lister]** 元件時。 因此，您AEM執行個體上可用的所有現有調適型表單都會列在頁面上。
1. 插入 **[!UICONTROL Drafts & Submissions]** 元件時。 兩個標籤， **[!UICONTROL Draft Forms]** 和 **[!UICONTROL Submitted Forms]**，即會顯示在Forms Portal頁面上。 此 **[!UICONTROL Draft Forms]** 索引標籤也會顯示使用中提及的步驟所產生的轉換後的最適化表單。 [設定已轉換的Forms Portal整合適用性表單](#configure-converted-adaptive-form-for-forms-portal-integration)

1. 點選 **[!UICONTROL Preview]**，點選轉換後的自適應表單，指定自適應表單欄位的值並提交它。 您為最適化表單欄位指定的值會提交至整合式資料庫。
